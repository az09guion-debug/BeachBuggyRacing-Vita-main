...
extern "C" void port_trace(const char *format, ...);
...
AAssetManager * AAssetManager_fromJava_soloader(void *env, void *assetManager) {
    (void)env;
    (void)assetManager;
...
AAsset* AAssetManager_open(AAssetManager* mgr, const char* filename, int mode) {
    ...
    if (!a->f) {
        port_trace("AssetManager: open failed path=%s mode=%d", realp.c_str(),
                   mode);
        free(a->filename);
        delete a;
        a = nullptr;
...
#endif
        a->opened = true;
        port_trace("AssetManager: opened path=%s size=%u mode=%d",
                   realp.c_str(), (unsigned)a->fileSize, mode);
    }
